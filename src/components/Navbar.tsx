      ...
      <div className="container-custom mx-auto">
        <nav className="flex items-center justify-between py-4">
          ...
          <Link to="/" className="flex items-center gap-2" onClick={closeMenu}>
            <span className="text-2xl font-display font-semibold text-zinc-300">
          ...
          <div className="hidden md:flex items-center gap-8">
            <ul className="flex items-center gap-6">
              {navLinks.map(link => <li key={link.title}>
                  <Link to={link.path} className="text-primary/80 hover:text-primary font-medium transition-colors duration-200 text-sm relative after:absolute after:bottom-0 after:left-0 after:h-0.5 after:w-0 after:bg-secondary after:transition-all hover:after:w-full">
                    {link.title}
                  </Link>
                </li>)}
            </ul>
          ...
          {/* Mobile Menu Button */}
          <button className="md:hidden text-primary p-1" onClick={toggleMenu} aria-label="Toggle Menu">
        ...
        <div className={`fixed inset-0 bg-white z-40 transform transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "translate-x-full"} md:hidden flex flex-col pt-20 px-6`}>
          <ul className="flex flex-col gap-5">
            {navLinks.map(link => <li key={link.title}>
                <Link to={link.path} className="text-primary text-lg font-medium block py-2 border-b border-gray-100" onClick={closeMenu}>
                  {link.title}
                </Link>
              </li>)}
          </ul>
